...
func MarkdownFileNameForPageID(pageID string) string {
	return markdownFileName("-", pageID)
}
...
func (c *Converter) RenderCollectionView(block *notionapi.Block) {
	...
	for f := 0; f < block.TableViews[0].ColumnCount(); f++ {
		c.Printf("|%s",  block.TableViews[0].Columns[f].Name())
	}
	c.Printf("|\n")
	for f := 0; f < block.TableViews[0].ColumnCount(); f++ {
		c.Printf("|---")
	...
	c.Printf("|\n")
	for row := 0; row < block.TableViews[0].RowCount(); row++ {
		for col := 0; col < block.TableViews[0].ColumnCount(); col++ {
			c.Printf("|")
			c.renderTableCell(block.TableViews[0], row, col)
		}
		c.Printf("|\n")
	}
	c.RenderChildren(block)
...
func (c *Converter) renderTableCell(tv *notionapi.TableView, row, col int) {
	...
	colVal := c.GetInlineContent(textSpans, false)
	...
	if typ == notionapi.ColumnTypeTitle {
		if isEmptyBlock(rowPage) {
			...
			colVal = fmt.Sprintf(`[%s](%s)`, uri, colVal)
		}
...
func (c *Converter) DefaultRenderFunc(blockType string) func(*notionapi.Block) {
	...
	case notionapi.BlockBreadcrumb:
		// TODO: NYI
	case notionapi.BlockCopyIndicator:
		// TODO: NYI
	case notionapi.BlockLinkToCollection:
		// TODO: NYI
	case notionapi.BlockMiro:
		// TODO: NYI
...
// RenderBlock renders a block to markdown
